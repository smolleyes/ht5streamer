...
// *** UPNP SERVICE WRAPPER ***
  ...
  UPnP.prototype.createRequest = function( upnpServiceType, upnpActionName, parameters ) {
    ...
    var svcMsg = this.requestTemplate.replace(/\{ACTION_TYPE\}/g, trim(upnpServiceType));

    svcMsg = svcMsg.replace(/\{ACTION_NAME\}/g, trim(upnpActionName));
    ...
    if( svcInParams ) {
      ...
      for(var svcParam in svcInParams) {
        ...
        svcInParams_xml += "\t\t\t<" + svcParam + ">" + trim(svcInParams[ svcParam ]['value'] + "") + "</" + svcParam + ">\n";
  ...
  UPnP.prototype.sendRequest = function( upnpServiceType, upnpActionName, upnpEndpointURL, upnpRequestXML, onUPnPResponse ) {
    ...
    try {
      ...
      xhr.onreadystatechange = function () {
        ...
        try {
			relTime = this.responseText.match(/<RelTime>(.*?)<\/RelTime>/)[1];
			trackDuration = this.responseText.match(/<TrackDuration>(.*?)<\/TrackDuration>/)[1];
		} catch(err) {}
