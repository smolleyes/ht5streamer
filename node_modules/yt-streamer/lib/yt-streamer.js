            ...
            if(err) {
				...
				for(var i=0; i<num; i++) {
					...
					if (in_array(resolution,res) === true || container == 'flv' || container == '3gp' ) {
						if (i === num-1) {
							analyseRes(resolutions,video,info);
						}
						continue;
					}
					res.push(resolution);
					resolutions[resolution] = [];
					resolutions[resolution]['itag'] = info.fmt_list[i][0];
...
function analyseRes(resolutions,video) {
	...
	Object.keys(resolutions).forEach(function(key) {
		...
		if (obj.cypher === false) {
			...
			getInfos(video,obj,key);
		}
	});
}

function getInfos(video,obj,res) {
	try {
		var options = ["--max-quality="+obj.itag];
