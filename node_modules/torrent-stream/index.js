...
var torrentStream = function(link, opts) {
	...
	if ((typeof(link) === 'string') && (link.indexOf('magnet:?xt') !== -1)) { 
    console.log("link is magnet")
    link = magnet(link);
  } else if(Buffer.isBuffer(link)) {
    console.log("link is web link")
    link = parseTorrent(link);
  } else {
    console.log("link is local file")
    link = link;
  }
	...
	if (!opts.blocklist) opts.blocklist = new Array();
	...
	var metadataPieces = new Array();
	var metadata = null;
	var refresh = noop;

	Engine.path = opts.path;
	Engine.files = new Array();
	Engine.selection = new Array();
	Engine.torrent = null;
	...
	var ontorrent = function(torrent) {
		...
		var onrequest = function(wire, index, hotswap) {
			...
			var r = reservations[index] || new Array();
		...
		wire.once('extended', function(id, handshake) {
			...
			wire.on('extended', function(id, ext) {
				...
				if (message.msg_type === 1 && !metadata) {
					...
					result['announce-list'] = new Array();
